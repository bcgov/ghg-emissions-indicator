...
-   **A number of gases are released into the atmosphere through different activities.** Different gases have different Global Warming Potential (GWP: [British Columbia Greenhouse Gas Provincial Inventory Methodology](https://www2.gov.bc.ca/assets/gov/environment/climate-change/data/provincial-inventory/2021/provincial_inventory_of_greenhouse_gas_emissions_1990-2021_-_methodology_report.pdf), for example, Methane (CH~4~) has a radiative forcing 25x that of Carbon dioxide(CO~2~), while Sulphur hexaflouride (SF~6~) has that of 22,880x more than CO~2~. For the puropose of this report, all emissions have been converted to units of CO~2~ equivalent (MtCO~2~e).
...
-   **B.C. applies forest management carbon offsets to emission totals each year to determine net greenhouse gas emissions and progress to its official legislated targets.** In 2021, carbon offsets totaled `r offset_val_Mtco2e` Mt CO~2~e, bringing net greenhouse gas emissions down to `r ghg_est_Mtco2e - offset_val_Mtco2e` Mt CO~2~e, `r round(((net_2007 - (ghg_est_Mtco2e - offset_val_Mtco2e))/net_2007)*100,0)`% below 2007 levels. Figures and calculations presented in this indicator use gross totals from the [British Columbia Greenhouse Gas Emission Inventory (`r max_ghg_yr`)](https://www2.gov.bc.ca/gov/content/environment/climate-change/data/provincial-inventory)^2^ and do not account for any emission reductions from offset projects.
...
-   A total of 6 greenhouse gases: Carbon dioxide, Hydroflourocarbons, Methane, Nitrous oxide, Perfluorocarbons and Sulphur hexafluoride, are monitored across B.C. Each of these gases has a different Global Warming Potential (GWP, provided in the [British Columbia Greenhouse Gas Provincial Inventory Methodology](https://www2.gov.bc.ca/assets/gov/environment/climate-change/data/provincial-inventory/2021/provincial_inventory_of_greenhouse_gas_emissions_1990-2021_-_methodology_report.pdf)). For the purpose of the below figures, values for each greenhouse gas have been converted to their CO~2~ equilvalent, allowing for comparison of effect size.

-   Greenhouse gas emissions between 1990 and 2021 have on average consisted of 81.3% CO~2~ (range = 80.3 - 84.1 %), and this percentage has stayed relatively consistent across this time period. 

-   Hydroflourocarbons (HFCs) have proportionally increased between 1995-2021, peaking in 2021 at 2.64% of greenhouse gas emissions.

-   Perflourocarbons (PFCs) have gradually declined from a high of 2.8% in 1998 to approximately 0.2% of greenhouse gas emmissions in 2021. 

-   Sulphur hexaflouride (SF~6~), which has the highest radiative forcing potential of all recorded gases, has varied between 0.1 and 0.2% of emmissions, and has not been included as a significant component of emmission since 2010. 
...
p = ggplotly(ghg_gases_prop, tooltip="text", dynamicTicks = TRUE) %>%
...
ggplotly(ghg_net_1990, tooltip="text", dynamicTicks = TRUE) %>%
  rangeslider(start = 1990, end = 2021)
```
...
include_graphics('images_for_HTML/LightIndustry.png')
