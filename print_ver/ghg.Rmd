...
title: "Trends in Greenhouse Gas Emissions in B.C. (1990-2020)"
output: 
...
-   **The decline in emissions observed in 2020 occurred during the COVID-19 pandemic, which reduced transportation and other activities that produce greenhouse gas emissions.** B.C.'s emissions estimates suggest that they are likely to have returned to pre-COVID levels in 2021 as economic activity (including air travel and road transportation) rebounded (see page 10 of [B.C.'s 2022 Climate Change Accountability Report](https://www2.gov.bc.ca/assets/gov/environment/climate-change/action/cleanbc/2022-ccar/2022_climate_change_accountability_report.pdf)^1^).

-   **B.C. applies carbon offsets to emission totals each year to determine net greenhouse gas emissions and progress to its official legislated targets.** In 2020, carbon offsets totaled `r offset_val_Mtco2e` Mt CO~2~e, bringing net greenhouse gas emissions down to `r ghg_est_Mtco2e - offset_val_Mtco2e` Mt CO~2~e, 3% below 2007 levels. Figures and calculations presented in this indicator use gross totals from the [British Columbia Greenhouse Gas Emission Inventory (`r max_ghg_yr`)](https://www2.gov.bc.ca/gov/content/environment/climate-change/data/provincial-inventory)^2^ and do not account for any emission reductions from offset projects.
...
-   **Greenhouse gas emissions per person continue to fall, despite a brief increase from 2015 - 2018.** Greenhouse gas emissions per person in B.C. consistently declined from 2004 to 2015, increased somewhat from 2015 to 2018, and have since decreased. Greenhouse gas emissions per unit gross domestic product have consistently declined over the past two decades.
...
```{r ghgtrends, fig.alt="Greenhouse gas emissions plotted over time from 1990 to 2020. The 2007 baseline and the 2025 emission target are highlighted on the graph."}
...
ggplot(data = bc_ghg_sum, aes(x = year, y = ghg_estimate)) + 
  ...
  geom_segment(aes(x = 2005, xend = 2007, y = 62, yend = 65.5), size = 0.5) +
  annotate("text", x=2018.5, y=clean_bc_2025, label="B.C. 2025 emission target", size = 4)+
  annotate("text", x=2004, y=61, label = "2007 baseline", size = 4)+
...
-   Overall, the tonnes of carbon dioxide equivalent (t CO~2~e) emitted per person in B.C. were less in 2020 than the 2007 baseline year.

\vspace{.2cm}

```{r ghgpop, fig.alt="Greenhouse gas emissions per capita plotted over time from 1990 to 2020"}
ghg_pop + theme_print + 
  labs(caption = 
...
```{r ghggdp, fig.alt="Greenhouse gas emissions per million dollars of GDP plotted over time from 1990 to 2020."}
...
```{r norm, fig.alt="GDP, population, and greenhouse gas emissions are plotted as three separate lines from 1990 to 2020."}
norm_print + 
  theme_print
```
...
```{r include=is_html, eval=is_html, fig.height=5, fig.width=11, fig.alt= "An interactive plot shows the greenhouse gas emissions from each economic sector from 1990 to 2020."}
...
-   Deforestation emissions declined consistently since 1990. Heavy industry's emissions decreased consistently from 2004 to 2013, with slight increases observed between 2015 and 2018, and a recent decline.
...
```{r include=is_html,  eval=is_html, results='asis', fig.alt= "A tabbed interactive plot that shows a breakdown of greenhouse gas emissions by subsectors within a sector from 1990 to 2020." }
...
The [British Columbia Greenhouse Gas Provincial Inventory Methodology](https://www2.gov.bc.ca/assets/gov/environment/climate-change/data/provincial-inventory/2019/provincial_inventory_methodology_book_1990-2019.pdf)^3^ provides a description of the methodologies and data sources used in preparing B.C.'s greenhouse gas emissions inventory. Currently, most of the Provincial Inventory data comes from [Canada's National Inventory Report (NIR)](http://www.publications.gc.ca/site/eng/9.506002/publication.html).^4^ In addition to using the NIR data, the B.C. Provincial Inventory generally follows the same methods and uses the same categories. Any deviations from the NIR methodology or categorization is described in detail in the B.C. Provincial Inventory Methodology^3^. Additional information is also available in the [British Columbia Greenhouse Gas Emission Inventory Report (2020)](https://www2.gov.bc.ca/gov/content/environment/climate-change/data/provincial-inventory)^2^.
